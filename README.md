...
This repository walks-through how to train a TinyYOLO model using Darkflow (Darknet to tensorflow translation). It shows how to load trained weights, retrain using tensorflow on own dataset, export constant graph def to mobile devices and finally validate those object of interest. 
...
The retrained graph have the naming cnvention of tiny-yolo-voc-xc.pb, which can be generated using --savepb argument as shown below
...
The output file is saved as video.avi under main folder (replacing the above video.avi generated by real-time validation)
...
Very clear documentation on how to convert a retrained graph using coreml and make it compactible for mobile device are elaborated in these below repositories. I just followed their step and validated using iOS device. 
